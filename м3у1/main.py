...
@app.route("/")
def main():
    return '<p>Привет, это сайт про технологические зависимости.</p> <a href="/random_fact">Посмотреть случайный факт!</a> <a href="/бросок октаэдр">бросок октаэдр!</a> '
...
@app.route("/бросок октаэдр")
def money():
    return f'<p>{random.choice(orelilireshka)}</p>'
